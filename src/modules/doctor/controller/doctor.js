...
export const doctors = async (req, res, next) => {
     const doctors = await doctorModel.find().select(' -password -confirmEmail -emailCode -verifyEmail -EmailPasswordCode -isLogin -createdAt -updatedAt')
    .populate({ path: 'patientId', select: 'firstName lastName email  phone_one guardianIds' })
...
//update doctor byId 'edit profile'
export const updateDoctor = asyncHandler(async (req, res, next) => {
...
export const logoutDoctor =asyncHandler( async (req, res, next) => {
    ...
    const {doctorId}=req.params
    const doctor = await doctorModel.updateOne({ _id:doctorId }, { status: 'offline', isLogin: 'false' })
...
const {doctorId}=req.params
    const patientDocID = await patientModel.find({ doctorId: doctorId})
